...
''' This is the version of the RL model where I am trying new reward calculations to acheive a faster lap time than 
model_prev_reward.py achieved. At the moment, I have not been able to train the model to complete a full lap with 
new reward calculations, but I am still trying. The number of iterations that this model has trained on is stored in 
iterations.pkl, though I reset it whenever I change the reward calculation. 
'''
# Initialize Pygame
pygame.init()
...
# Try lap while train flag. If True, the car tries a lap based on the current Q-table after every training iteration
# without modifying the Q-table
# If False, the car only trains and modifies the Q-table on every iteration
try_lap_while_train = True
