...
''' This is the version of the car driving simulator that the user can play like a game. The user uses the arrow keys
to drive the car. The physics of the car are the same as in the RL model. If the user completes a full lap and the lap
time is faster than the previous best lap time, the best lao time updates and is stored in best_lap_time_user.pkl
'''
# Initialize Pygame
pygame.init()
...
if os.path.exists('Car_Driving_RL_Model/best_lap_time_user.pkl'):
    with open('Car_Driving_RL_Model/best_lap_time_user.pkl', 'rb') as f:
...
# Draws the arrow keys and shows when each key is pressed
...
while running:
    ...
    # Does not start timer until car begins moving
    ...
    # Changes the car's motion based on arrows being pressed
    if active_key == pygame.K_UP:
    ...
    # If best lap time has been achieved, records lap time and resets the car
    if lap_completed:
        lap_time = time.time() - lap_start_time
        if best_lap_time is None or lap_time < best_lap_time:
            best_lap_time = lap_time
            with open('Car_Driving_RL_Model/best_lap_time_user.pkl', 'wb') as f:
